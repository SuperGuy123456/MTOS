...
void RunApp::DrawScreen()
{
    ...
    // Run button
    API::DrawRaw("/os/img/run.raw", runX, runY, 64, 64);
...
void RunApp::DrawDescriptionWrapped(const String& text, int x, int y, int maxWidth)
{
    String line;
    String word;
...
void RunApp::HandleTouch()
{
    ...
    // Run button
    if (tx > runX && tx < runX + runW &&
        ty > runY && ty < runY + runH)
    {
        String wasmPath = apps[selectedApp].folder + "/app.wasm";
        lcd.fillScreen(TFT_BLACK);
        Wasm::Run(wasmPath.c_str());
        API::DrawText("Exiting...", 0, 0, TFT_RED, TFT_BLACK, 4, false, true);
